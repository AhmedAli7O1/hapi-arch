...
function loadModels (pluginData) {
  try {
    if (pluginData.folders.indexOf('models') >= 0)
      pluginData.models = 
        modulesLoader.loadAllWithFun(
          path.join(pluginData.path, 'models'),
          (modulePath, moduleName) => db.model(moduleName, require(modulePath)));
    ...
    if (pluginData.folders.indexOf('services') >= 0)
      pluginData.services = 
        modulesLoader.loadAllWithFun(
          ...
          module => require(module)(server, options, pluginData.models, methods));
    ...
    if (pluginData.folders.indexOf('controllers') >= 0)
      pluginData.controllers = 
        modulesLoader.loadAllWithFun(
          ...
          module => require(module)(server, options, pluginData.services, methods));
...
function loadSchema (pluginData) {
  try { 
    if (pluginData.folders.indexOf('schema') >= 0)
      pluginData.schema = 
        modulesLoader.loadAll(path.join(pluginData.path, 'schema'));
